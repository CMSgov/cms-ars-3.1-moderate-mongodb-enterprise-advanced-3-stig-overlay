...
include_controls 'mongodb-enterprise-advanced-3-stig-baseline' do
  control 'V-81847' do
    title 'MongoDB must provide audit record generation for CMS-defined auditable events within all DBMS/database components.'
    desc 'MongoDB must provide audit record generation capability for CMS-defined auditable events within all DBMS/database components.
    Satisfies:
    SRG-APP-000089-DB-000064, SRG-APP-000080-DB-000063, SRG-APP-000090-DB-000065, 
  ...
  control 'V-81865' do
    title 'If DBMS authentication, using passwords, is employed, MongoDB must enforce the CMS standards for password complexity and lifetime.'
    ...
    The CMS standard for authentication is CMS-approved PKI certificates. Authentication based on User ID and Password may be used only when it is not possible to employ a PKI certificate, and requires AO approval.

    In such cases, the CMS standards for password complexity and lifetime must be implemented. DBMS products that can inherit the rules for these from the operating system or access control program (e.g., Microsoft Active Directory) must be configured to do so. For other DBMSs, the rules must be enforced using available configuration parameters or custom code.'
  end
  ...
  control 'V-81871' do
    ...
    If the private key is stolen, an attacker can use the private key(s) to impersonate the certificate holder. In cases where MongoDB-stored private keys are used to authenticate MongoDB to the system’s clients, loss of the corresponding private keys would allow an attacker to successfully perform undetected man in the middle attacks against MongoDB system and its clients.
  ...
  control 'V-81873' do
    desc 'The CMS standard for authentication is CMS-approved PKI certificates. Once a PKI certificate has been validated, it must be mapped to a DBMS user account for the authenticated identity to be meaningful to MongoDB and useful for authorization decisions.'
  end
  ...
  control 'V-81907' do
    title 'MongoDB must provide a warning to appropriate support staff when allocated audit record storage volume reaches 80% of maximum audit record storage capacity.'
    desc 'Organizations are required to use a central log management system, so, under normal conditions, the audit space allocated to MongoDB on its own server will not be an issue. 
    However, space will still be required on MongoDB server for audit records in transit, and, under abnormal conditions, this could fill up. Since a requirement exists to halt processing upon audit failure, a service outage would result.
    ...
    Configure the required alert in the monitoring software to send an alert where storage volume holding the auditLog file utilization reaches 80%.'
  end
  ...
  control 'V-81917' do
    impact 0.0
    title 'MongoDB must only accept end entity certificates issued by CMS PKI or CMS-approved PKI Certification Authorities (CAs) for the establishment of all encrypted sessions.'
    ...
    tag 'check', 'To run MongoDB in SSL mode, you have to obtain a valid certificate singed by a single certificate authority. 

    Before starting the MongoDB database in SSL mode, verify that certificate used is issued by a valid CMS certificate authority (openssl x509 -in <path_to_certificate_pem_file> -text | grep -i "issuer"). 

    If there is any issuer present in the certificate that is not a CMS approved certificate authority, this is a finding.'
    tag 'fix', 'Remove any certificate that was not issued by an approved CMS certificate authority. Contact the organization\'s certificate issuer and request a new certificate that is issued by a valid CMS certificate authorities.'

    desc 'caveat', 'Not applicable for this CMS ARS 3.1 overlay, since the related security control is not included in CMS ARS 3.1'
  end

  control 'V-81923' do
    desc 'fix', '"Obtain a certificate from a valid CMS certificate authority to be used for encrypted data transmission. 
  ...
  control 'V-81929' do
    title 'MongoDB must be configured in accordance with the security configuration settings based on CMS security configuration and implementation guidance, including STIGs, NSA configuration guides, CTOs, DTMs, and IAVMs.'
    desc 'Configuring MongoDB to implement organization-wide security implementation guides and security checklists ensures compliance with federal standards and establishes a common security baseline across CMS that reflects the most restrictive security posture consistent with operational requirements.
    ...
    desc 'check', 'Review the MongoDB documentation and configuration to determine it is configured in accordance with CMS security configuration and implementation guidance, including STIGs, NSA configuration guides, CTOs, DTMs, and IAVMs.
